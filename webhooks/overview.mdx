...
<Frame>![Image](/images/image.png)</Frame>
...
<Frame>![Download](/images/download.png)</Frame>
...
- `webhook`: The webhook that was triggered.
...
<Tabs>
  ...
  <Tab title="Connector Sync Error">
    ...
    ```json
    {
      ...
      "error": {
        "failed_at": "2026-06-29T13:17:31.085343+00:00",
        "message": "Received error response when validating the connection.",
        "detail": {
          "status_code": 500,
          "message": "invalid username or password"
        } // or {} if no detail is available
      }
    }
    ```
...
> Retries are not triggered based on HTTP status codes. If your server responds with any `4xx` or `5xx` status code, Bindbee treats the request as completed and does not retry the webhook.
